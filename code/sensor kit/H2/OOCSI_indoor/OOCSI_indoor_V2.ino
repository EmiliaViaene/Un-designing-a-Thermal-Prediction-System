...
const char* OOCSIName = "HH2_indoor_data_V2.2";
...
void processOOCSI() {
  float Temperature_HH2 = oocsi.getFloat("Temperature", -200.0);
  String Participant_HH2 = oocsi.getString("participant", "");

  if (Temperature_HH2 > -30.0 && Participant_HH2 == "H2") { // only return the temperature if its value is not rogue & specify which household I want the data from
...
void loop() { 
  ...
  float temperature_corrected = temperature - 3;
  ...
  oocsi.addString("device_id", "d20e71f9031b04884");
  ...
  // replace DEVICE with your reference ID from a device
  oocsi.addString("device_id", "dcc7e03d212484140");
  // identify participant
  oocsi.addString("participant", "H2");
