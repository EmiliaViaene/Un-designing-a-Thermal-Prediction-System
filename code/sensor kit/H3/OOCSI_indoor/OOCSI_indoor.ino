...
const char* OOCSIName = "HH3_indoor_data";
...
void processOOCSI() {
  float Temperature_HH3 = oocsi.getFloat("Temperature", -200.0);
  String Participant_HH3 = oocsi.getString("participant", "");

  if (Temperature_HH3 > -30.0 && Participant_HH3 == "H3") { // only return the temperature if its value is not rogue & specify which household I want the data from
...
void loop() { 
  ...
  float temperature_corrected = temperature - 2.7;
  ...
  oocsi.addString("device_id", "d1583e8bf757149f0");
  ...
  // replace DEVICE with your reference ID from a device
  oocsi.addString("device_id", "db13a3330148d420a");
  // identify participant
  oocsi.addString("participant", "H3");
